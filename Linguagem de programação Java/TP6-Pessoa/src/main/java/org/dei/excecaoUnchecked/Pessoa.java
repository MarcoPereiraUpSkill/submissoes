...
public class Pessoa implements Comparable<Pessoa> {
    ...
    public void setNome(String nome) {
        try {
            if (nome.length() == 0) {
                throw new ArgumentoForaDosLimitesException(nome + " não é um nome válido");
            }
    ...
    public void setIDCivil(int idCivil) {
        try {
            if (idCivil > 99999999 || idCivil < 0) {
                throw new ArgumentoForaDosLimitesException(idCivil + " não é um número de ID Civil válido");
            }

            this.idCivil = idCivil;
        } catch (ArgumentoForaDosLimitesException e) {
            System.out.println(e.getMessage());
            this.idCivil = IDCIVIL_POR_OMISSAO;
        }
    }
