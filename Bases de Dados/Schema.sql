...
/*
insert into participa(id_atleta, id_competicao)
select 'insert into Participa(id_atleta, id_competicao) values('||a.id||', '||b.id||');'
  from atleta a, competicao b
 where exists(select * from atividadeatleta where id_atleta = a.id and id_atividade = b.id_atividade)
 order by 1;
*/
commit;